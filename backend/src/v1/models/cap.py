...
if TYPE_CHECKING:
    ...
    from src.v1.models.basins import BasinBase  
...
class Cap_Event_Base(SQLModel):
    ...
    cap_event_status_id: int = Field(
        default=None,
        foreign_key=f"{default_schema}.cap_event_status.cap_event_status_id",
    )
...
class Cap_Event(Cap_Event_Read, table=True):
    ...
    cap_event_status: Optional["Cap_Event_Status"] = Relationship(
        back_populates="cap_event_status_lnk"
    )
...
class Cap_Event_Areas_Read(Cap_Event_Areas_Base):
    cap_event_id: int = Field(
        default=None, foreign_key=f"{default_schema}.cap_event.cap_event_id"
    )
    basin_id: int = Field(default=None, foreign_key=f"{default_schema}.basins.basin_id")
...
class Cap_Event_History(SQLModel, table=True):
    ...
    cap_event_hist_created_date: datetime.datetime = datetime.datetime.now(
    ...
    cap_event_areas_hist: List["Cap_Event_Areas_History"] = Relationship(
        back_populates="events_history"
    )
    cap_event_status: Optional["Cap_Event_Status"] = Relationship(
        back_populates="cap_event_hist_status_lnk"
    )
...
class Cap_Event_Areas_History(SQLModel, table=True):
    ...
    cap_event_history_id: int = Field(
        default=None,
        foreign_key=f"{default_schema}.cap_event_history.cap_event_history_id",
    )
    basin_id: int = Field(default=None, foreign_key=f"{default_schema}.basins.basin_id")

    basins: List["Basins"] = Relationship(back_populates="basin_cap_hist_links")
    events_history: "Cap_Event_History" = Relationship(
        back_populates="cap_event_areas_hist"
    )
...
class Cap_Event_Status(SQLModel, table=True):
    ...
    cap_event_status: str
    # cap_event_status
    cap_event_status_lnk: Optional["Cap_Event"] = Relationship(
        back_populates="cap_event_status"
    )
    cap_event_hist_status_lnk: Optional["Cap_Event_History"] = Relationship(
        back_populates="cap_event_status"
    )
